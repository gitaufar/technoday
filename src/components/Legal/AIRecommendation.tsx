...
// Tipe data diperbarui untuk menyertakan 'low'
...
// Tipe data saran juga diperbarui
type AiSuggestion = {
  title: string;
...
function SuggestionModal({ suggestion, onClose }: { suggestion: AiSuggestion; onClose: () => void }) {
  const styles = {
    ...
    low: { background: 'bg-green-50', text: 'text-green-800' }, // Ditambahkan
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={onClose}>
      <div className="relative w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4">
          ...
          <div className={`mt-1 rounded-md p-3 text-sm ${currentStyle.background} ${currentStyle.text}`}>
            {suggestion.originalContent}
          </div>
        </div>
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase text-green-700">AI Suggestion ✨</p>
          <div className="mt-1 rounded-md border border-green-200 bg-green-50 p-3 text-sm text-green-900">
            {suggestion.suggestedContent}
          </div>
        </div>
        <div className="mt-6 flex items-center justify-end gap-3">
          <button className="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">
            Reject
          </button>
          <button className="flex items-center gap-2 rounded-md bg-blue-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-blue-700">
...
export default function InteractiveDocumentViewer({ documentTitle, sections, suggestions }: DocumentViewerProps) {
  ...
  const highlightStyles = {
    ...
    low: 'bg-green-100 border border-green-200 rounded-md p-2 cursor-pointer hover:bg-green-200', // Ditambahkan
  };
  ...
  return (
    <>
      <div className="max-w-4xl rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <div className="p-6">
          <div className="rounded-md bg-gray-50 p-8 shadow-inner">
            <h1 className="mb-10 text-center text-xl font-bold uppercase tracking-wider text-gray-900">
              {documentTitle}
            </h1>
            <div className="space-y-6">
              {sections.map((section, index) => (
                <div key={index}>
                  ...
                  <p
                    className={`mt-1 text-gray-700 ${section.highlight ? highlightStyles[section.highlight] : ''}`}
